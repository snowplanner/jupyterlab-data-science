...
def load_json_file (path_file_json):
    """
      Loads a geojson file
      Parameters:
        path_file_json (string): path where it is stored the geojson file
    """
    with open(path_file_json, 'r') as function:
        trails_json = json.loads(function.read())
    return trails_json


def load_geom_json (trails_json):
    """
      Loads the set of geometries of a geojson file
      Parameters:
        trails_json (json): json file loaded into python
    """
    trails_geom = [feature['geometry'] for feature in trails_json['features']]
    return trails_geom
...
def write_geojson (input_file, output_path):
    """
      ...
      Parameters:
        input_file (json): json file loaded into python
